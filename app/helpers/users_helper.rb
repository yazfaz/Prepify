  ...
  def progress_report(courses)    
    ...
    courses.sort.each do |course|
      if course.completed_at.nil?        
        ...
        report += content_tag(:h3, class: 'incomplete panel') { "#{course.subject.name}: You are #{user_progress.round}% Completed" }
        report += progress_bar(user_progress)
      else
        report += content_tag(:h3, class: 'complete panel') {  "#{course.subject.name}: Completed!" }
      end
    end
    report.html_safe
  end
